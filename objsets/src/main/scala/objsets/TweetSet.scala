...
abstract class TweetSet extends TweetSetInterface {
  ...
  /**
   * This method checks whether TweetSet is empty or not.
   */
  def isEmpty(): Boolean
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val accThis = if (p(elem)) acc.incl(elem) else acc
    right.filterAcc(p, left.filterAcc(p, accThis))
  }
  ...
  override def mostRetweeted: Tweet = {
    ...
    else if (right.isEmpty() && !left.isEmpty()) maxTweet(elem, left.mostRetweeted)
